...
def train(model, model_loss, optimizer, TrainImgLoader, TestImgLoader, start_epoch, args):
    if args.lr_scheduler == 'MS':
        ...
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 0.9 ** (
                (epoch - 1) / len(TrainImgLoader)), last_epoch=len(TrainImgLoader) * start_epoch - 1)
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    logger.info(
                        "Epoch {}/{}, Iter {}/{}, 2mm_err={:.3f} | lr={:.6f}, train_loss={:.3f}, abs_err={:.3f}, pw_loss={:.3f}, pp_loss={:.3f}, dds_loss={:.3f}, dr_loss={:.3f}, time={:.3f}".format(
                            ...
                            scalar_outputs["s3_pp_loss"],
                            scalar_outputs["s3_dds_loss"],
                            scalar_outputs["s3_dr_loss"],
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        logger.info(
                            "Epoch {}/{}, Iter {}/{}, 2mm_err={:.3f} | lr={:.6f}, test_loss={:.3f}, abs_err={:.3f}, pw_loss={:.3f}, pp_loss={:.3f}, dds_loss={:.3f}, dr_loss={:.3f}, time={:.3f}".format(
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    outputs = model(
        ...
        sample_cuda["ref_lbp_mask"],
        sample_cuda["mask"]
    )

    depth_est = outputs["depth"]

    loss, epe_depth_pre, epe_depth_refine, pw_loss_stages, dds_loss_stages, pp_loss_stages, dr_loss_stages = model_loss(
    ...
    scalar_outputs = {
        "loss": loss,
        "epe_depth_pre": epe_depth_pre,
        "epe_depth_refine": epe_depth_refine,
        ...
        "s0_pp_loss": pp_loss_stages[0],
        "s1_pp_loss": pp_loss_stages[1],
        "s2_pp_loss": pp_loss_stages[2],
        "s3_pp_loss": pp_loss_stages[3],
        "s1_dr_loss": pp_loss_stages[0],
        "s2_dr_loss": pp_loss_stages[1],
        "s3_dr_loss": pp_loss_stages[2],
...
if __name__ == '__main__':
    ...
    model = GeoMVSNet(
        ...
        var_reg_dim_stages=[int(n) for n in args.var_reg_dim_stages.split(",")],
    )
    model.to(device)
    ...
    if args.which_dataset == "dtu":
        train_dataset = DTUDataset(args.trainpath, args.trainlist, "train", args.n_views, data_scale=args.data_scale,
        ...
        train_dataset = BlendedMVSDataset(args.trainpath, args.trainlist, "train", args.n_views, img_wh=(768, 576),
                                          robust_train=args.robust_train, augment=False)
