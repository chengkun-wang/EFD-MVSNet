...
class ConvWithFilter(nn.Module):
    ...
    def forward(self, features, filters):
        ...
        out_W = int(out_W)
        # (B,4,H,W)->(B,4*49,H,W)
        features_unf = torch.nn.functional.unfold(features, (self.kernel_size, self.kernel_size),
...
class GeoFeatureFusion(nn.Module):
    def __init__(self, convolutional_layer_encoding="z", mask_type="basic", add_origin_feat_flag=True):
        ...
        # rgb encoder
        self.filter_gen_1 = FilterGen(in_ch=1, conv_in_ch=4, conv_out_ch=4, kernel_size=5, stride=1, padding=2)
        self.conv_with_filter_1 = ConvWithFilter(out_ch=4, kernel_size=5, stride=1, padding=2)
        ...
        # depth encoder
        self.filter_gen_2 = FilterGen(in_ch=1, conv_in_ch=2, conv_out_ch=2, kernel_size=5, stride=1, padding=2)
        self.conv_with_filter_2 = ConvWithFilter(out_ch=2, kernel_size=5, stride=1, padding=2)
        ...
        # output
        self.output_depth_layer1 = convbnrelu(in_channels=32, out_channels=2, kernel_size=3, stride=1, padding=1)
        self.output_depth_layer2 = convbnrelu(in_channels=16, out_channels=2, kernel_size=3, stride=1, padding=1)
        self.output_depth_layer3 = convbnrelu(in_channels=8, out_channels=2, kernel_size=3, stride=1, padding=1)

        self.rgbdepth_decoder_stage1 = deconvbnrelu(in_channels=32, out_channels=32, kernel_size=5, stride=2, padding=2,
                                                    output_padding=1)
        self.rgbdepth_decoder_stage2 = deconvbnrelu(in_channels=16, out_channels=16, kernel_size=5, stride=2, padding=2,
                                                    output_padding=1)
        self.rgbdepth_decoder_stage3 = deconvbnrelu(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1,
                                                    output_padding=0)

        self.final_decoder_stage1 = deconvbnrelu(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1,
                                                 output_padding=0)
        self.final_decoder_stage2 = deconvbnrelu(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1,
                                                 output_padding=0)
    ...
    def forward(self, rgb, depth, confidence, depth_values, stage_idx, origin_feat, intrinsics_matrices_stage,
        ...
        conv_mask = (~text_mask).float().unsqueeze(1)
        ...
        # print(self.rgb_conv_init[0].weight.grad)
        # print(self.rgb_conv_init[0].weight.detach())
        d_mask_filter_1 = self.filter_gen_1(conv_mask)
        conv_with_filter_1 = self.conv_with_filter_1(torch.cat((rgb, d), dim=1), d_mask_filter_1)
        rgb_feature = self.rgb_conv_init(conv_with_filter_1)  # b 8 h w
        ...
        # -----------------------------------------------------------------------------------------
        mask_filter_2 = self.filter_gen_2(conv_mask)
        con_with_filter_2 = self.conv_with_filter_2(torch.cat((d, rgb_depth), dim=1), mask_filter_2)
        sparsed_feature = self.depth_conv_init(con_with_filter_2)  # b 8 h w
        ...
        if stage_idx >= 1:
            ...
            if stage_idx == 1:
                ...
                depth_output = self.output_depth_layer1(rgbdepth_feature)
        ...
        if stage_idx >= 2:
            ...
            if stage_idx == 2:
                ...
                depth_output = self.output_depth_layer2(rgbdepth_feature)
                d_depth, d_conf = torch.chunk(depth_output, 2, dim=1)
                rgb_conf, d_conf = torch.chunk(self.softmax(torch.cat((rgb_conf, d_conf), dim=1)), 2, dim=1)
                output = rgb_conf * rgb_depth + d_conf * d_depth
                if self.add_origin_feat_flag:
        ...
        if stage_idx >= 3:
            ...
            if stage_idx == 3:
                ...
                depth_output = self.output_depth_layer3(rgbdepth_feature)
                d_depth, d_conf = torch.chunk(depth_output, 2, dim=1)
                rgb_conf, d_conf = torch.chunk(self.softmax(torch.cat((rgb_conf, d_conf), dim=1)), 2, dim=1)
                output = rgb_conf * rgb_depth + d_conf * d_depth
                if self.add_origin_feat_flag:
        ...
        refine_depth = output * (depth_max - depth_min) + depth_min

        return final_feature, refine_depth
...
class Reg_BasicBlockGeo(nn.Module):
    ...
    def forward(self, x, g1=None, g2=None):
        identity = x
        if g1 is not None:
            x = torch.cat((x, g1), 1)  # x = torch.mul(x,g1)

        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        if g2 is not None:
            out = torch.cat((g2, out), 1)  # out = torch.mul(g2,out)
