    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:
                ...
                # @Note depth & mask
                if self.mode in ["train", "val"]:
                    depth_filename_hr = os.path.join(self.root_dir, 'Depths_raw/Depths/{}/depth_map_{:0>4}.pfm'.format(scan, vid))
                    depth = self.read_depth_hr(depth_filename_hr, scale_ratio)

                    mask_filename_hr = os.path.join(self.root_dir, 'Depths_raw/Depths/{}/depth_visual_{:0>4}.png'.format(scan, vid))
